...
const Head = () => {
  ...
  const getSearchSuggestions = async () => {
    ...
    const data = await fetch(YOUTUBE_API_SEARCH + searchQuery);
  ...
  return (
    <div className="flex items-center justify-between shadow-lg">
      ...
      <div className="flex items-center">
        <input
          className="w-96 border border-gray-400 rounded-l-full h-[34px] p-3"
          type="text"
          value={searchQuery}
        ...
        {showSuggestion && (
          <div className="absolute w-[430px] top-16 bg-white">
            <ul>
              {suggestion.map((data) => (
                <li className="m-[10px] cursor-pointer">{data}</li>
              ))}
            </ul>
          </div>
        )}
